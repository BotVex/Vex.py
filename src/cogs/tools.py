...
class Tools(commands.Cog):
    ...
    @commands.slash_command(
        name=Localized("user", key="TOOLS_USER_NAME"), dm_permission=True
    )
    async def user(self, inter: ACI):
        pass

    @commands.slash_command(
        name=Localized("server", key="TOOLS_SERVER_NAME"), dm_permission=False
    )
    async def server(self, inter: ACI):
        pass

    @commands.slash_command(
        name=Localized("qrcode", key="TOOLS_QRCODE_NAME"), dm_permission=True
    )
    async def qrcode(self, inter: ACI):
        pass

    @commands.slash_command(
        name=Localized("convert", key="TOOLS_CONVERT_NAME"), dm_permission=True
    )
    async def convert(self, inter: ACI):
        pass

    @commands.slash_command(
        name=Localized("emoji", key="TOOLS_EMOJI_NAME"), dm_permission=True
    )
    ...
    @server.sub_command(
        ...
        description=Localized(
            "Gets the server icon.", key="TOOLS_SERVER_CMD_ICON_DESC"
        ),
    )
    async def icon(self, inter: ACI):
        ...
        if server_icon != None:
            ...
            await inter.send(
                f"{Emojis.ERROR} O servidor não possuí um ícone.", ephemeral=True
            )
            return
    ...
    @server.sub_command(
        ...
        description=Localized(
            "Get the server banner.", key="TOOLS_SERVER_CMD_BANNER_DESC"
        ),
    )
    async def banner(self, inter: ACI):
        ...
        if server_banner != None:
            ...
            await inter.response.send_message(
                f"{Emojis.ERROR} O servidor não possuí um banner.", ephemeral=True
            )
            return
    ...
    @user.sub_command(
        ...
        description=Localized(
            "Gets the user's avatar.", key="TOOLS_USER_CMD_AVATAR_DESC"
        ),
        option=[
            disnake.Option(
                name="user",
    ...
    async def avatar(
        self, inter: ACI, user: Union[disnake.User, disnake.Member] = None
    ):
        ...
        class Views(disnake.ui.View):
            ...
            @disnake.ui.button(
                label=":globe_with_meridians: Ver avatar global",
                style=disnake.ButtonStyle.blurple,
            )
            async def view_global_avatar(
    ...
    @user.sub_command(
        ...
        description=Localized(
            "Gets the user's banner.", key="TOOLS_USER_CMD_BANNER_DESC"
        ),
        option=[
            disnake.Option(
                name="user",
    ...
    async def banner(self, inter: ACI, user: disnake.User = None):
        ...
        if user_fetch.banner is not None:
            ...
            await inter.send(
                f"{Emojis.ERROR} {user.display_name} não possui um banner",
                ephemeral=True,
            )
    ...
    @user.sub_command(
        ...
        description=Localized(
            "Gets information about the user.", key="TOOLS_USER_CMD_INFO_DESC"
        ),
        options=[
            disnake.Option(
                name="user",
                description=Localized(
                    "The user to see the information.", key="TOOLS_USER_CMD_INFO_USER"
                ),
                type=disnake.OptionType.user,
                required=False,
            )
    ...
    async def info(self, inter: ACI, user: Union[disnake.User, disnake.Member] = None):
        ...
        user_embed.add_field(
            name=":identification_card: Nome:", value=username, inline=True
        )
        user_embed.add_field(
            name=":jigsaw: Discriminador:", value=user_discrim, inline=True
        )
        user_embed.add_field(name=":id: ID:", value=user_id, inline=True)
        user_embed.add_field(
            name=":timer: Conta criada em:", value=account_created, inline=True
        )
        ...
        if isinstance(user, disnake.Member):
            ...
            if member.nick is not None:
                member_embed.add_field(
                    name=":bookmark: Apelido no servidor:",
                    value=member.nick,
                    inline=True,
                )

            member_embed.add_field(
                name=":clock: Entrou no servidor em:", value=joined_date, inline=True
            )
            ...
            if member.current_timeout is not None:
                ...
                member_embed.add_field(
                    name=":probing_cane: De castigo até:", value=timestamp, inline=True
                )

            if member.top_role.name != "@everyone":
                member_embed.add_field(
                    name=":arrow_up: Maior cargo:",
            ...
            if member.voice is not None:
                member_embed.add_field(
                    name=":telephone_receiver: Em call em:",
            ...
            if member.premium_since is not None:
                ...
                member_embed.add_field(
                    name=f"{Emojis.BOOST} Impulsionando o servidor desde:",
                    value=premium_since_time,
                    inline=True,
                )
        ...
        if user.bot is True:
            ...
            bot_is_public_string = (
                ":white_check_mark: " if response["bot_public"] is True else ":x: "
            ) + "Bot público"
            bot_requires_oauth2_string = (
                ":white_check_mark: "
                if response["bot_require_code_grant"] is True
                else ":x: "
            ...
            if response["icon"] is not None:
                ...
                bot_embed.color = await GetColor.general_color_url(
                    application_icon + "?size=16"
                )
                bot_embed.set_thumbnail(url=application_icon)

            bot_embed.description = response["description"]
            bot_embed.add_field(
                name=":zany_face: Outros:", value=bot_other_info, inline=False
            )
            bot_embed.add_field(
                name=":key: Chave Pública de Verificação de Requisições HTTP:",
                value=verify_key,
                inline=False,
            )
    ...
    @qrcode.sub_command(
        ...
        options=[
            disnake.Option(
                name="text",
                description=Localized(
                    "Text of qrcode.", key="TOOLS_QRCODE_CMD_CREATE_TEXT"
                ),
    ...
    @convert.sub_command(
        ...
        description=Localized(
            "Convert temperature.", key="TOOLS_CONVERT_CMD_TEMPERATURE_DESC"
        ),
        options=[
            disnake.Option(
                name="value",
                description=Localized(
                    ...
                    key="TOOLS_CONVERT_CMD_TEMPERATURE_FROM_VALUE_DESC",
                ),
            ...
            disnake.Option(
                ...
                description=Localized(
                    ...
                    key="TOOLS_CONVERT_CMD_TEMPERATURE_FROM_SCALE_DESC",
                ),
                type=disnake.OptionType.string,
                required=True,
                choices=[
                    disnake.OptionChoice(
                        name=Localized("Celcius", key="CELCIUS_NAME"), value="Celcius"
                    ),
                    disnake.OptionChoice(
                        name=Localized("Fahrenheit", key="FAHRENHEIT_NAME"),
                        value="Fahrenheit",
                    ),
                    disnake.OptionChoice(
                        name=Localized("Kelvin", key="KELVIN_NAME"), value="Kelvin"
                    ),
                ],
            ),
            disnake.Option(
                ...
                description=Localized(
                    "The target scale.",
                    key="TOOLS_CONVERT_CMD_TEMPERATURE_TO_SCALE_DESC",
                ),
                type=disnake.OptionType.string,
                required=True,
                choices=[
    ...
    async def temperature(self, inter: ACI, value: int, scale: str, to_scale: str):
        ...
        if to_scale == scale:
            await inter.send(
                f"{Emojis.ERROR} Você não pode converter uma temperatura para ela mesma.",
                ephemeral=True,
            )
            return
            ...
            color = await GetColor.general_color_url(
                inter.author.display_avatar.with_size(16)
            )
            ...
            embed.description = (
                f'{str(value)}{"°" if scale in ["Celcius", "Fahrenheit"] else ""}'
                + scale[0:1]
                + " = "
                + f'{str(round(conversion, 2))}{"°" if to_scale in ["Celcius", "Fahrenheit"] else ""}'
                + to_scale[0:1]
            )
            embed.timestamp = datetime.datetime.now()
            embed.set_footer(
