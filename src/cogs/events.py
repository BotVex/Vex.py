...
class Events(commands.Cog):
	...
	async def default_color_task(self):
		...
		hex_color = ColorConverter.DECIMAL2HEX(self.bot.default_color).replace('0x', '#')
	...
	async def on_connect(self):
		...
		log.success(f'starting presence [slate_blue1]{activity_name}[/] defined in all shards')
			...
			if len(mentions) != 0:
				for mentioned in mentions:
					if mentioned.id == self.bot.user.id:
						...
						log.info(f'[deep_pink1]{message.author}[/] ({message.author.id}) mentioned in {mentioned_local}')
						...
						try:
							...
							log.error(f'falied to reply mention of {message.author} ({message.author.id})')
	...
	async def status_task(self):
		...
		for shard_id in [x for x in self.bot.shards]:
			...
			log.success(f'presence [slate_blue1]{activity_name}[/] defined in all shards')
	...
	async def on_ready(self):
		...
		log.success(f'[cyan1]{self.bot.user}[/] [bold green]online[/] - [yellow1]{len(self.bot.guilds)} servers[/]')
	...
	async def on_slash_command(self, inter: ACI):
		log.info(f'[deep_pink1]{inter.author}[/] ({inter.author.id}) executed [dark_orange bold]/{inter.application_command.qualified_name}[/]')
