...
class Image_(commands.Cog):
    ...
    async def autocontrast(
        ...
        if file.content_type not in [
            ...
            await inter.send(
                embed=EB(
                    title=f"{E.unknown_file} | formato de arquivo não suportado.",
                    color=C.error,
                )
            )
            return
            ...
            await inter.send(
                embed=EB(
                    title=f"{E.error} | o arquivo é muito grante.",
            ...
            img = F.autocontrast(
                await file.read(), cutoff, ignore, preserve_tone=preserve_tone
            )
    ...
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def equalize(self, inter: ACI, file: disnake.Attachment):
        await inter.response.defer()
        if file.content_type not in [
            "image/bmp",
            "image/jpeg",
    ...
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def flip(self, inter: ACI, file: disnake.Attachment):
        await inter.response.defer()
        if file.content_type not in [
            "image/bmp",
            "image/jpeg",
    ...
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def mirror(self, inter: ACI, file: disnake.Attachment):
        await inter.response.defer()
        if file.content_type not in [
            "image/bmp",
            "image/jpeg",
    ...
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def invert(self, inter: ACI, file: disnake.Attachment):
        await inter.response.defer()
        if file.content_type not in [
            "image/bmp",
            "image/jpeg",
    ...
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def grayscale(self, inter: ACI, file: disnake.Attachment):
        await inter.response.defer()
        if file.content_type not in [
            "image/bmp",
            "image/jpeg",
    ...
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def posterize(self, inter: ACI, file: disnake.Attachment, bits: int = 3):
        await inter.response.defer()
        if file.content_type not in [
            "image/bmp",
            "image/jpeg",
    ...
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def solarize(self, inter: ACI, file: disnake.Attachment, threshold: int = 20):
        await inter.response.defer()
        if file.content_type not in [
            "image/bmp",
            "image/jpeg",
    ...
    async def unsharp(
        ...
        if file.content_type not in [
            ...
            img = F.unsharp(
                await file.read(), radius=radius, percent=percent, threshold=threshold
            )
    ...
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def blur(self, inter: ACI, file: disnake.Attachment, intensity: int = 10):
        await inter.response.defer()
        if file.content_type not in [
            "image/bmp",
            "image/jpeg",
    ...
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def pixelize(
        self, inter: ACI, file: disnake.Attachment, pixels: int, resize: bool = True
    ):
        await inter.response.defer()
        if file.content_type not in [
            "image/bmp",
