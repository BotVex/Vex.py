...
class Entertainment(commands.Cog):
    ...
    @fun.sub_command(
        ...
        description=Localized(
            "Do an roleplay with someone.", key="ENT_FUN_CMD_ROLEPLAY_DESC"
        ),
        options=[
            disnake.Option(
                name="user",
                description=Localized(
                    "Choice a user.", key="ENT_FUN_CMD_ROLEPLAY_USERDESC"
                ),
            ...
            disnake.Option(
                name="roleplay",
                description=Localized(
                    "Choice a roleplay.", key="ENT_FUN_CMD_ROLEPLAY_ROLEPLAYDESC"
                ),
                choices=[
                    disnake.OptionChoice(
                        Localized("highfive", key="ENT_FUN_CMD_ROLEPLAY_HIGHFIVE"),
                        "highfive",
                    ),
                    disnake.OptionChoice(
                        Localized("handhold", key="ENT_FUN_CMD_ROLEPLAY_HANDHOLD"),
                        "handhold",
                    ),
                    disnake.OptionChoice(
                        Localized("kiss", key="ENT_FUN_CMD_ROLEPLAY_KISS"), "kiss"
                    ),
                    disnake.OptionChoice(
                        Localized("wave", key="ENT_FUN_CMD_ROLEPLAY_WAVE"), "wave"
                    ),
                    disnake.OptionChoice(
                        Localized("thumbsup", key="ENT_FUN_CMD_ROLEPLAY_THUMBSUP"),
                        "thumbsup",
                    ),
                    disnake.OptionChoice(
                        Localized("stare", key="ENT_FUN_CMD_ROLEPLAY_STARE"), "stare"
                    ),
                    disnake.OptionChoice(
                        Localized("baka", key="ENT_FUN_CMD_ROLEPLAY_BAKA"), "baka"
                    ),
                    disnake.OptionChoice(
                        Localized("wink", key="ENT_FUN_CMD_ROLEPLAY_WINK"), "wink"
                    ),
                    disnake.OptionChoice(
                        Localized("shoot", key="ENT_FUN_CMD_ROLEPLAY_SHOOT"), "shoot"
                    ),
                    disnake.OptionChoice(
                        Localized("shrug", key="ENT_FUN_CMD_ROLEPLAY_SHRUG"), "shrug"
                    ),
                    disnake.OptionChoice(
                        Localized("kick", key="ENT_FUN_CMD_ROLEPLAY_KICK"), "kick"
                    ),
                    disnake.OptionChoice(
                        Localized("hug", key="ENT_FUN_CMD_ROLEPLAY_HUG"), "hug"
                    ),
                    disnake.OptionChoice(
                        Localized("slap", key="ENT_FUN_CMD_ROLEPLAY_SLAP"), "slap"
                    ),
                    disnake.OptionChoice(
                        Localized("pat", key="ENT_FUN_CMD_ROLEPLAY_PAT"), "pat"
                    ),
                    disnake.OptionChoice(
                        Localized("punch", key="ENT_FUN_CMD_ROLEPLAY_PUNCH"), "punch"
                    ),
                    disnake.OptionChoice(
                        Localized("dance", key="ENT_FUN_CMD_ROLEPLAY_DANCE"), "dance"
                    ),
                    disnake.OptionChoice(
                        Localized("bite", key="ENT_FUN_CMD_ROLEPLAY_BITE"), "bite"
                    ),
                ],
    ...
    async def roleplay(self, inter: ACI, user: disnake.User, roleplay: str):
        ...
        embed.set_footer(
            text=f"⛲ Fonte: {name} (by nekos.best) | {inter.author.display_name}",
        ...
        match roleplay:
            ...
            case "wave":
                message = (
                    f":wave: | {inter.author.mention} acenou para  {user.mention}!"
                )
            case "thumbsup":
            ...
            case "hug":
                message = (
                    f":hugging_face: | {inter.author.mention} abraçou {user.mention}!"
                )
            case "slap":
            ...
            case "punch":
                message = (
                    f":punch: | {inter.author.mention} deu um soco em {user.mention}!"
                )
            case "dance":
                message = f"{Emojis.DANCE} | {inter.author.mention} dançou com {user.mention}!"
            case "bite":
                message = (
                    f":fork_and_knife: | {inter.author.mention} mordeu {user.mention}!"
                )
            case "shoot":
        ...
        if inter.author.id == user.id:
            match roleplay:
                ...
                case "handhold":
                    message = (
                        f":handshake: | {inter.author.mention} segurou sua própia mão?"
                    )
                case "kiss":
            ...
            async def retribue(
                ...
                if interaction.author.id != user.id:
                    await interaction.send(
                        f"{Emojis.ERROR} Ei!, apenas {user.mention} pode usar isso!",
                        ephemeral=True,
                    )
                else:
                    self.value = True
        ...
        if view.value is True:
            ...
            embed_retribued = EB(color=color, timestamp=datetime.datetime.now())

            embed_retribued.set_footer(
                text=f"⛲ Fonte: {name} (by nekos.best) | {user.display_name}",
                icon_url=user.display_avatar,
            )
            embed_retribued.set_image(url=url)

            await inter.send(
                content=f"{Emojis.REVERSE} {inter.author.mention}, {user.mention} Retribuiu!",
                embed=embed_retribued,
            )
    ...
    @fun.sub_command(
        ...
        description=Localized(
            "Generate a custom kaomoji.", key="ENT_FUN_CMD_KAOMOJI_DESC"
        ),
        options=[
            disnake.Option(
                name="category",
                description=Localized(
                    "Choose a category.", key="ENT_FUN_CMD_KAOMOJI_DESC"
                ),
                choices=[
                    disnake.OptionChoice(
                        Localized(
                            "neutral", key="ENT_FUN_CMD_KAOMOJI_CATEGORY_CHOICE_NEUTRAL"
                        ),
                        "neutral",
                    ),
                    disnake.OptionChoice(
                        Localized(
                            "happy", key="ENT_FUN_CMD_KAOMOJI_CATEGORY_CHOICE_HAPPY"
                        ),
                        "happy",
                    ),
                    disnake.OptionChoice(
                        Localized(
                            "love", key="ENT_FUN_CMD_KAOMOJI_CATEGORY_CHOICE_LOVE"
                        ),
                        "love",
                    ),
                    disnake.OptionChoice(
                        Localized("sad", key="ENT_FUN_CMD_KAOMOJI_CATEGORY_CHOICE_SAD"),
                        "sad",
                    ),
                    disnake.OptionChoice(
                        Localized(
                            "random", key="ENT_FUN_CMD_KAOMOJI_CATEGORY_CHOICE_RANDOM"
                        ),
                        "random",
                    ),
                ],
