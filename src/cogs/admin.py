...
class Administration(commands.Cog):
    ...
    @commands.slash_command(
        name=Localized("adm", key="ADM_ADM_NAME"), dm_permission=False
    )
    ...
    @adm.sub_command(
        ...
        description=Localized(
            "Deletes the specified number of messages.", key="ADM_MOD_CMD_PURGE_DESC"
        ),
        options=[
            disnake.Option(
                name="amount",
                description=Localized(
                    "The number of messages that will be deleted. It must be between 2 and 1000.",
                    key="ADM_MOD_CMD_PURGE_AMOUNT_DESC",
                ),
    ...
    async def botme(
        self, inter: ACI, message: str, channel: disnake.TextChannel = None
    ):
        await inter.response.defer()
        ...
        for webhook in channel_webhooks:
            if webhook.user == self.bot.user and webhook.name == "Vex Webhook":
                break
        else:
            webhook = await channel.create_webhook(name="Vex Webhook")
...
# 				embed = EB(color=DefaultColors.SUCCESS)
# 				embed.title = 'nick alterado!'
...
# 				embed = EB(color=DefaultColors.ERROR)
# 				embed.title = 'Erro!'
