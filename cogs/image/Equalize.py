...
class Equalize(commands.Cog):
	...
	@commands.command(
		name='equalizar',
		description=f'*tento equlizar sua foto automaticamente*',
		aliases=[
			'equa'
			])
	async def equlize(self, ctx):
		...
		try:
			...
			img_cont = ImageOps.equalize(img_obj, mask=None)
			img_cont.save('dataequalized.png', 'png')
			color_thief = ColorThief('dataequalized.png')
			dominant_color = color_thief.get_color(quality=1)
			material = ''.join(f'{i:02X}' for i in dominant_color)
			file = disnake.File('dataequalized.png', filename='autocontrast.png')
			os.remove('dataequalized.png')
