...
class Gray(commands.Cog):
	...
	@commands.command(
		name='gray',
		description=f'*deixo sua imgem cinza*',
		aliases=[
			'grayscale',
			'cinza'
			])
	async def gray(self, ctx):
		...
		try:
			...
			img_cont = ImageOps.grayscale(img_obj)
			img_cont.save('datagray.png', 'png')
			color_thief = ColorThief('datagray.png')
			dominant_color = color_thief.get_color(quality=1)
			material = ''.join(f'{i:02X}' for i in dominant_color)
			file = disnake.File('datagray.png', filename='autocontrast.png')
			os.remove('datagray.png')
