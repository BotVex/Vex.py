...
class Flip(commands.Cog):
	...
	@commands.command(
		name='flip',
		description=f'*viro sua imagem verticalmente*',
		aliases=[
			'virar'
			])
	async def equlize(self, ctx):
		...
		try:
			...
			img_cont = ImageOps.flip(img_obj)
			img_cont.save('dataflipped.png', 'png')
			color_thief = ColorThief('dataflipped.png')
			dominant_color = color_thief.get_color(quality=1)
			material = ''.join(f'{i:02X}' for i in dominant_color)
			file = disnake.File('dataflipped.png', filename='autocontrast.png')
			os.remove('dataflipped.png')
