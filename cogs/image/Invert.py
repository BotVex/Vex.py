...
class Invert(commands.Cog):
	...
	@commands.command(
		name='inverter',
		description=f'*eu vou inverter as cores da sua imagem*',
		aliases=[
			'invert'
			])
	async def invert(self, ctx):
		...
		try:
			...
			img_cont = ImageOps.invert(img_obj)
			img_cont.save('datainverted.png', 'png')
			color_thief = ColorThief('datainverted.png')
			dominant_color = color_thief.get_color(quality=1)
			material = ''.join(f'{i:02X}' for i in dominant_color)
			file = disnake.File('datainverted.png', filename='autocontrast.png')
			os.remove('datainverted.png')
