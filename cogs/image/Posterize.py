...
class Posterize(commands.Cog):
	...
	@commands.command(
		name='posterizar',
		description=f'*eu vou posterizar sua imagem*',
		aliases=[
			'posterize',
			'post'
			])
	async def posterize(self, ctx):
		...
		try:
			...
			img_cont = ImageOps.posterize(img_obj, 2)
			img_cont.save('dataposterized.png', 'png')
			color_thief = ColorThief('dataposterized.png')
			dominant_color = color_thief.get_color(quality=1)
			material = ''.join(f'{i:02X}' for i in dominant_color)
			file = disnake.File('dataposterized.png', filename='autocontrast.png')
			os.remove('dataposterized.png')
