...
class Solarize(commands.Cog):
	...
	@commands.command(
		name='solarizar',
		description=f'*eu vou solarizar sua imagem*',
		aliases=[
			'solarize',
			'solar'
			])
	async def solarize(self, ctx):
		...
		try:
			...
			img_cont = ImageOps.solarize(img_obj)
			img_cont.save('data/solarized.png', 'png')
			color_thief = ColorThief('data/solarized.png')
			dominant_color = color_thief.get_color(quality=1)
			material = ''.join(f'{i:02X}' for i in dominant_color)
			file = disnake.File('data/solarized.png', filename='autocontrast.png')
			os.remove('data/solarized.png')
