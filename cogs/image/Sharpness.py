...
class Unsharp(commands.Cog):
	...
	@commands.command(
		name='nitidez',
		description=f'*tento aumentar a nitidez automaticamente*',
		aliases=[
			'sharpness'
			])
	async def sharpness(self, ctx):
		...
		try:
			...
			img_cont = img_obj.filter(ImageFilter.UnsharpMask())
			img_cont.save('datasharpness.png', 'png')
			color_thief = ColorThief('datasharpness.png')
			dominant_color = color_thief.get_color(quality=1)
			material = ''.join(f'{i:02X}' for i in dominant_color)
			file = disnake.File('datasharpness.png', filename='autocontrast.png')
			os.remove('datasharpness.png')
