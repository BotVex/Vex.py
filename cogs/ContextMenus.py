...
class CTXCMD(commands.Cog):
	...
	@commands.user_command(name=Localized('Spotify', key='CTXMENUS_USERCMD_SPOTIFY_NAME'), dm_permission=False)
	async def spotifysong(self, inter: disnake.CmdInter, user: disnake.Member):
			...
			embed = EB(
				title=spotify.title,
				description=f"Por: {''.join(spotify.artists) if len(spotify.artists) == 1 else ', '.join(spotify.artists)}",
